...
if (_key != "" && isPlayer _player && {_isGenStore || _isGunStore || _isVehStore}) then
{
	...
	if (!isNil "_itemEntry" && {{_x == _marker} count allMapMarkers > 0}) then
	{
		...
		if (_player getVariable ["cmoney", 0] >= _itemPrice) then
		{
			...
			_object setVariable ["ownerN", name _player, true]; //Cael817, Add "ownerName" for convenience, changed to ownerN for the condom guy
			_object setVariable ["R3F_LOG_disabled", false, true]; //Cael817, So R3F_LOG 3.x works
			...
			if (_object isKindOf "AllVehicles" && !(_object isKindOf "StaticWeapon")) then
			{
				...
				_object lock 2;  // modificado tabareco
				//_object lock 2; //Cael817, Spawn vehicle locked
				_object engineOn true; //Cael817, Spawn vehicle with engine on
				_object setVariable ["R3F_LOG_disabled", true, true]; //Cael817, Spawn vehicle untowable/liftable
			};
			//Cael817, if UAV or UGV don't spawn it locked or untowable/liftable
			...
			switch (true) do
			{			
				//Cael817, Add food and water to bought food sacks and barrels.
